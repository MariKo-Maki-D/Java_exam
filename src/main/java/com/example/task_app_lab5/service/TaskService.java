...
public class TaskService {
    ...
    public Page<Tasks> getTasksForUser(Long userId, String search, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        if (search == null || search.isEmpty()) {
            return taskRepo.findByUserId(userId, pageable);
        }
        return taskRepo.findByUserIdAndTitleContainingIgnoreCase(userId, search, pageable);
    }
